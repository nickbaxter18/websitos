...
class ModelEmbedder:
    ...
    def embed_models_strict(
        ...
        if parallel is None or parallel == 1 or is_small:
            for batch in iter_batch(raw_models, batch_size):
                yield from self.embed_models_batch(
                    batch, inference_batch_size=batch_size
                )
        else:
            multiprocessing_batch_size = (
                1  # larger batch sizes do not help with data parallel
            )
    ...
    def _accumulate(self, data: models.VectorStruct) -> None:
        ...
        if isinstance(data, list):
            for value in data:
                if not isinstance(
                    value, get_args(INFERENCE_OBJECT_TYPES)
    ...
    def _drain_accumulator(
        ...
        if not self._embed_storage or not self._embed_storage.get(data.model, None):
            self._embed_accumulator(
                is_query=is_query, inference_batch_size=inference_batch_size
            )

        return self._next_embed(data.model)

    def _embed_accumulator(
        self, is_query: bool = False, inference_batch_size: int = 8
    ) -> None:
        ...
        def embed(
            ...
            unique_options_is_text: list[bool] = (
                []
            )  # multimodal models can have both text
            ...
            embeddings = []
            for i, (options, is_text) in enumerate(
                zip(unique_options, unique_options_is_text)
            ):
                embeddings.extend(
                    [
