...
class ModelSchemaParser:
    ...
    def _replace_refs(
        ...
        if isinstance(schema, dict):
            ...
            if "properties" in schema:
                ...
                for k, v in schema.items():
                    parent_key = (
                        k if isinstance(v, dict) and "properties" in v else parent
                    )
                    schemes[k] = self._replace_refs(
        ...
        for key, value in schema.items():
            ...
            if key == "$ref":
                ...
                if (
                    ...
                    self.name_recursive_ref_mapping[current_path.split(".")[-1]] = (
                        model_name
                    )

            if (
                after_properties
            ):  # field name seen in pydantic models comes after "properties" key
                if current_path:
    ...
    def parse_model(self, model: Type[BaseModel]) -> None:
        ...
        for ref in self._recursive_refs:
            if (
                ref in self._excluded_recursive_refs
                or ref in self._included_recursive_refs
            ):
                continue
    ...
    def _persist(self, output_path: Union[Path, str] = CACHE_PATH) -> None:
        ...
        with open(output_path, "w") as f:
            ...
            f.write(
                f"INCLUDED_RECURSIVE_REFS = {sorted(self._included_recursive_refs)}\n"
            )
            f.write(
                f"EXCLUDED_RECURSIVE_REFS = {sorted(self._excluded_recursive_refs)}\n"
            )
