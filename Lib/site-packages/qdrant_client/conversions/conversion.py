...
def has_field(message: Any, field: str) -> bool:
    ...
    try:
        ...
        all_fields = set(
            [descriptor.name for descriptor, _value in message.ListFields()]
        )
...
def json_to_value(payload: Any) -> Value:
    ...
    if isinstance(payload, dict):
        return Value(
            struct_value=Struct(
                fields=dict((k, json_to_value(v)) for k, v in payload.items())
            )
        )
...
def value_to_json(value: Value) -> Any:
    ...
    if "structValue" in value_:
        ...
        return dict(
            ...
            for key, val in value_["structValue"]["fields"].items()
        )
...
class GrpcToRest:
    ...
    def convert_geo_radius(cls, model: grpc.GeoRadius) -> rest.GeoRadius:
        return rest.GeoRadius(
            center=cls.convert_geo_point(model.center), radius=model.radius
        )
    ...
    def convert_collection_info(cls, model: grpc.CollectionInfo) -> rest.CollectionInfo:
        return rest.CollectionInfo(
            ...
            vectors_count=(
                model.vectors_count if model.HasField("vectors_count") else None
            ),
    ...
    def convert_optimizer_status(
        cls, model: grpc.OptimizerStatus
    ) -> rest.OptimizersStatus:
        if model.ok:
    ...
    @classmethod
    def convert_collection_config(
        cls, model: grpc.CollectionConfig
    ) -> rest.CollectionConfig:
    ...
    @classmethod
    def convert_hnsw_config_diff(
        cls, model: grpc.HnswConfigDiff
    ) -> rest.HnswConfigDiff:
        return rest.HnswConfigDiff(
            ...
            full_scan_threshold=(
                model.full_scan_threshold
                if model.HasField("full_scan_threshold")
                else None
            ),
            max_indexing_threads=(
                model.max_indexing_threads
    ...
    def convert_optimizer_config(
        ...
        return rest.OptimizersConfig(
            ...
            flush_interval_sec=(
                model.flush_interval_sec
                if model.HasField("flush_interval_sec")
                else None
            ),
            indexing_threshold=(
                model.indexing_threshold
    ...
    def convert_wal_config(cls, model: grpc.WalConfigDiff) -> rest.WalConfig:
        return rest.WalConfig(
            wal_capacity_mb=(
                model.wal_capacity_mb if model.HasField("wal_capacity_mb") else None
            ),
            wal_segments_ahead=(
                model.wal_segments_ahead
    ...
    def convert_payload_schema(
        ...
        return {
            key: cls.convert_payload_schema_info(info) for key, info in model.items()
        }

    @classmethod
    def convert_payload_schema_info(
        cls, model: grpc.PayloadSchemaInfo
    ) -> rest.PayloadIndexInfo:
    ...
    def convert_payload_schema_params(
        ...
        raise ValueError(
            f"invalid PayloadIndexParams model: {model}"
        )  # pragma: no cover

    @classmethod
    def convert_payload_schema_type(
        cls, model: grpc.PayloadSchemaType
        ...
        if model == grpc.PayloadSchemaType.Float:
            ...
            raise ValueError(
                f"invalid PayloadSchemaType model: {model}"
            )  # pragma: no cover

    @classmethod
    def convert_collection_status(
        cls, model: grpc.CollectionStatus
    ) -> rest.CollectionStatus:
    ...
    @classmethod
    def convert_has_id_condition(
        cls, model: grpc.HasIdCondition
    ) -> rest.HasIdCondition:
    ...
    def convert_create_alias(cls, model: grpc.CreateAlias) -> rest.CreateAlias:
        return rest.CreateAlias(
            collection_name=model.collection_name, alias_name=model.alias_name
        )
    ...
    def convert_scored_point(cls, model: grpc.ScoredPoint) -> rest.ScoredPoint:
        return construct(
            ...
            payload=(
                cls.convert_payload(model.payload)
                if has_field(model, "payload")
                else None
            ),
            score=model.score,
            vector=(
                cls.convert_vectors_output(model.vectors)
                if model.HasField("vectors")
                else None
            ),
            version=model.version,
            shard_key=(
                cls.convert_shard_key(model.shard_key)
    ...
    @classmethod
    def convert_geo_bounding_box(
        cls, model: grpc.GeoBoundingBox
    ) -> rest.GeoBoundingBox:
    ...
    def convert_point_struct(cls, model: grpc.PointStruct) -> rest.PointStruct:
        return rest.PointStruct(
            ...
            vector=(
                cls.convert_vectors(model.vectors)
                if model.HasField("vectors")
                else None
            ),
        )
    ...
    def convert_field_condition(cls, model: grpc.FieldCondition) -> rest.FieldCondition:
        ...
        geo_radius = (
            cls.convert_geo_radius(model.geo_radius)
            if model.HasField("geo_radius")
            else None
        )

        geo_polygon = (
            cls.convert_geo_polygon(model.geo_polygon)
            if model.HasField("geo_polygon")
            else None
        )
    ...
    @classmethod
    def convert_collection_params(
        cls, model: grpc.CollectionParams
    ) -> rest.CollectionParams:
    ...
    def convert_update_collection(
        cls, model: grpc.UpdateCollection
    ) -> rest.UpdateCollection:
    ...
    @classmethod
    def convert_alias_operations(
        cls, model: grpc.AliasOperations
    ) -> rest.AliasOperations:
        name = model.WhichOneof("action")
        if name is None:
            raise ValueError(
                f"invalid AliasOperations model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    @classmethod
    def convert_alias_description(
        cls, model: grpc.AliasDescription
    ) -> rest.AliasDescription:
    ...
    def convert_points_selector(
        ...
        if name is None:
            raise ValueError(
                f"invalid PointsSelector model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    def convert_with_payload_selector(
        ...
        if name is None:
            raise ValueError(
                f"invalid WithPayloadSelector model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    def convert_vector_params(cls, model: grpc.VectorParams) -> rest.VectorParams:
        return rest.VectorParams(
            ...
            on_disk=model.on_disk if model.HasField("on_disk") else None,
            datatype=(
                cls.convert_datatype(model.datatype)
                if model.HasField("datatype")
                else None
            ),
            multivector_config=(
    ...
    @classmethod
    def convert_multivector_config(
        cls, model: grpc.MultiVectorConfig
    ) -> rest.MultiVectorConfig:
    ...
    def convert_multivector_comparator(
        ...
        raise ValueError(
            f"invalid MultiVectorComparator model: {model}"
        )  # pragma: no cover
    ...
    def convert_vectors_config(cls, model: grpc.VectorsConfig) -> rest.VectorsConfig:
        ...
        if name is None:
            raise ValueError(
                f"invalid VectorsConfig model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    @classmethod
    def _convert_vector(cls, model: Union[grpc.Vector, grpc.VectorOutput]) -> tuple[
        Optional[str],
        Union[
            list[float],
            list[list[float]],
        ...
        if name is None:
            if model.HasField("indices"):
                return None, rest.SparseVector(
                    indices=model.indices.data[:], values=model.data[:]
                )
            if model.HasField("vectors_count"):
                ...
                return None, [
                    vectors[i : i + step] for i in range(0, len(vectors), step)
                ]
    ...
    @classmethod
    def convert_vector(cls, model: grpc.Vector) -> Union[
        list[float],
        list[list[float]],
        rest.SparseVector,
        rest.Document,
    ...
    @classmethod
    def convert_vectors_output(
        cls, model: grpc.VectorsOutput
    ) -> rest.VectorStructOutput:
        name = model.WhichOneof("vectors_options")
        if name is None:
            raise ValueError(
                f"invalid VectorsOutput model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    @classmethod
    def convert_multi_dense_vector(
        cls, model: grpc.MultiDenseVector
    ) -> list[list[float]]:
    ...
    @classmethod
    def convert_inference_object(
        cls, model: grpc.InferenceObject
    ) -> rest.InferenceObject:
    ...
    def convert_context_input_pair(
        cls, model: grpc.ContextInputPair
    ) -> rest.ContextPair:
    ...
    def convert_sum_expression(cls, model: grpc.SumExpression) -> rest.SumExpression:
        return rest.SumExpression(
            sum=[cls.convert_expression(expr) for expr in model.sum]
        )

    @classmethod
    def convert_mult_expression(cls, model: grpc.MultExpression) -> rest.MultExpression:
        return rest.MultExpression(
            mult=[cls.convert_expression(expr) for expr in model.mult]
        )
    ...
    def convert_div_expression(cls, model: grpc.DivExpression) -> rest.DivExpression:
        ...
        by_zero_default = (
            model.by_zero_default if model.HasField("by_zero_default") else None
        )
    ...
    def convert_decay_params_expression(
        ...
        return rest.DecayParamsExpression(
            ...
            target=(
                cls.convert_expression(model.target)
                if model.HasField("target")
                else None
            ),
    ...
    def convert_mmr(cls, model: grpc.Mmr) -> rest.Mmr:
        return rest.Mmr(
            ...
            candidates_limit=(
                model.candidates_limit if model.HasField("candidates_limit") else None
            ),
        )
    ...
    def convert_query(cls, model: grpc.Query) -> rest.Query:
        ...
        if name == "nearest_with_mmr":
            ...
            return rest.NearestQuery(
                nearest=cls.convert_vector_input(val.nearest),
                mmr=cls.convert_mmr(val.mmr),
            )
    ...
    def convert_prefetch_query(cls, model: grpc.PrefetchQuery) -> rest.Prefetch:
        return rest.Prefetch(
            ...
            filter=(
                cls.convert_filter(model.filter) if model.HasField("filter") else None
            ),
            params=(
                cls.convert_search_params(model.params)
                if model.HasField("params")
                else None
            ),
            score_threshold=(
                model.score_threshold if model.HasField("score_threshold") else None
            ),
    ...
    def convert_with_vectors_selector(
        cls, model: grpc.WithVectorsSelector
    ) -> rest.WithVector:
        name = model.WhichOneof("selector_options")
        if name is None:
            raise ValueError(
                f"invalid WithVectorsSelector model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    def convert_recommend_points(
        cls, model: grpc.RecommendPoints
    ) -> rest.RecommendRequest:
        ...
        positive_vectors = [
            cls.convert_vector(vector) for vector in model.positive_vectors
        ]
        negative_vectors = [
            cls.convert_vector(vector) for vector in model.negative_vectors
        ]
    ...
    def convert_discover_points(
        cls, model: grpc.DiscoverPoints
    ) -> rest.DiscoverRequest:
        target = (
            cls.convert_target_vector(model.target)
            if model.HasField("target")
            else None
        )
    ...
    @classmethod
    def convert_text_index_params(
        cls, model: grpc.TextIndexParams
    ) -> rest.TextIndexParams:
        return rest.TextIndexParams(
            type="text",
            tokenizer=cls.convert_tokenizer_type(model.tokenizer),
            min_token_len=(
                model.min_token_len if model.HasField("min_token_len") else None
            ),
            max_token_len=(
                model.max_token_len if model.HasField("max_token_len") else None
            ),
            lowercase=model.lowercase if model.HasField("lowercase") else None,
            phrase_matching=(
                model.phrase_matching if model.HasField("phrase_matching") else None
            ),
            stopwords=(
                cls.convert_stopwords(model.stopwords)
                if model.HasField("stopwords")
                else None
            ),
            on_disk=model.on_disk if model.HasField("on_disk") else None,
            stemmer=(
                cls.convert_stemmer(model.stemmer)
                if model.HasField("stemmer")
                else None
            ),
        )
    ...
    def convert_stemmer(cls, model: grpc.StemmingAlgorithm) -> rest.StemmingAlgorithm:
        ...
        if name is None:
            raise ValueError(
                f"invalid StemmingAlgorithm model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    @classmethod
    def convert_float_index_params(
        cls, model: grpc.FloatIndexParams
    ) -> rest.FloatIndexParams:
    ...
    @classmethod
    def convert_bool_index_params(
        cls, model: grpc.BoolIndexParams
    ) -> rest.BoolIndexParams:
    ...
    @classmethod
    def convert_uuid_index_params(
        cls, model: grpc.UuidIndexParams
    ) -> rest.UuidIndexParams:
    ...
    def convert_collection_params_diff(
        ...
        return rest.CollectionParamsDiff(
            ...
            on_disk_payload=(
                model.on_disk_payload if model.HasField("on_disk_payload") else None
            ),
        )
    ...
    @classmethod
    def convert_read_consistency(
        cls, model: grpc.ReadConsistency
    ) -> rest.ReadConsistency:
        name = model.WhichOneof("value")
        if name is None:
            raise ValueError(
                f"invalid ReadConsistency model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
        if name == "factor":
            return val
    ...
    def convert_read_consistency_type(
        ...
        raise ValueError(
            f"invalid ReadConsistencyType model: {model}"
        )  # pragma: no cover
    ...
    def convert_binary_quantization_config(
        ...
        return rest.BinaryQuantizationConfig(
            always_ram=model.always_ram if model.HasField("always_ram") else None,
            encoding=(
                cls.convert_binary_quantization_encoding(model.encoding)
                if model.HasField("encoding")
                else None
            ),
            query_encoding=(
                cls.convert_binary_quantization_query_encoding(model.query_encoding)
    ...
    def convert_binary_quantization_encoding(
        ...
        raise ValueError(
            f"invalid BinaryQuantizationEncoding model: {model}"
        )  # pragma: no cover
    ...
    @classmethod
    def convert_compression_ratio(
        cls, model: grpc.CompressionRatio
    ) -> rest.CompressionRatio:
        if model == grpc.x4:
    ...
    def convert_quantization_config(
        ...
        if name is None:
            raise ValueError(
                f"invalid QuantizationConfig model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
        if name == "scalar":
            return rest.ScalarQuantization(
                scalar=cls.convert_scalar_quantization_config(val)
            )
        if name == "product":
            return rest.ProductQuantization(
                product=cls.convert_product_quantization_config(val)
            )
        if name == "binary":
            return rest.BinaryQuantization(
                binary=cls.convert_binary_quantization_config(val)
            )
        raise ValueError(
            f"invalid QuantizationConfig model: {model}"
        )  # pragma: no cover
    ...
    def convert_point_group(cls, model: grpc.PointGroup) -> rest.PointGroup:
        return rest.PointGroup(
            ...
            lookup=(
                cls.convert_record(model.lookup) if model.HasField("lookup") else None
            ),
        )
    ...
    def convert_quantization_config_diff(
        ...
        if name is None:
            raise ValueError(
                f"invalid QuantizationConfigDiff model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
        if name == "scalar":
            return rest.ScalarQuantization(
    ...
    @classmethod
    def convert_vector_params_diff(
        cls, model: grpc.VectorParamsDiff
    ...
    @classmethod
    def convert_vectors_config_diff(
        cls, model: grpc.VectorsConfigDiff
    ) -> rest.VectorsConfigDiff:
        name = model.WhichOneof("config")
        if name is None:
            raise ValueError(
                f"invalid VectorsConfigDiff model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
    ...
    def convert_points_update_operation(
        ...
        if name is None:
            raise ValueError(
                f"invalid PointsUpdateOperation model: {model}"
            )  # pragma: no cover
        val = getattr(model, name)
        ...
        if name == "upsert":
            ...
            raise ValueError(
                f"invalid UpdateOperation model: {model}"
            )  # pragma: no cover
    ...
    @classmethod
    def convert_recommend_strategy(
        cls, model: grpc.RecommendStrategy
    ) -> rest.RecommendStrategy:
        ...
        raise ValueError(
            f"invalid RecommendStrategy model: {model}"
        )  # pragma: no cover

    @classmethod
    def convert_sparse_index_config(
        cls, model: grpc.SparseIndexConfig
    ) -> rest.SparseIndexParams:
    ...
    def convert_sparse_vector_params(
        ...
        return rest.SparseVectorParams(
            ...
            modifier=(
                cls.convert_modifier(model.modifier)
                if model.HasField("modifier")
                else None
            ),
        )
    ...
    def convert_sparse_vector_config(
        ...
        return dict(
            (key, cls.convert_sparse_vector_params(val))
            for key, val in model.map.items()
        )
    ...
    @classmethod
    def convert_shard_key_selector(
        cls, model: grpc.ShardKeySelector
    ) -> rest.ShardKeySelector:
    ...
    def convert_order_by(cls, model: grpc.OrderBy) -> rest.OrderBy:
        return rest.OrderBy(
            key=model.key,
            direction=(
                cls.convert_direction(model.direction)
                if model.HasField("direction")
                else None
            ),
            start_from=(
                cls.convert_start_from(model.start_from)
    ...
    @classmethod
    def convert_health_check_reply(
        cls, model: grpc.HealthCheckReply
    ) -> rest.VersionInfo:
    ...
    @classmethod
    def convert_search_matrix_pair(
        cls, model: grpc.SearchMatrixPair
    ) -> rest.SearchMatrixPair:
    ...
    @classmethod
    def convert_strict_mode_sparse(
        cls, model: grpc.StrictModeSparse
    ) -> rest.StrictModeSparse:
    ...
    def convert_strict_mode_config(
        ...
        return rest.StrictModeConfig(
            ...
            search_max_hnsw_ef=(
                model.search_max_hnsw_ef
                if model.HasField("search_max_hnsw_ef")
                else None
            ),
            search_allow_exact=(
                model.search_allow_exact
            ...
            read_rate_limit=(
                model.read_rate_limit if model.HasField("read_rate_limit") else None
            ),
            write_rate_limit=(
    ...
    def convert_strict_mode_config_output(
        ...
        return rest.StrictModeConfigOutput(
            ...
            multivector_config=(
                cls.convert_strict_mode_multivector_config_output(
...
class RestToGrpc:
    ...
    def convert_filter(cls, model: rest.Filter) -> grpc.Filter:
        ...
        return grpc.Filter(
            ...
            must_not=(
                convert_conditions(model.must_not)
                if model.must_not is not None
                else None
            ),
            should=(
                convert_conditions(model.should) if model.should is not None else None
            ),
            min_should=(
                grpc.MinShould(
    ...
    def convert_collection_info(cls, model: rest.CollectionInfo) -> grpc.CollectionInfo:
        return grpc.CollectionInfo(
            config=(
                cls.convert_collection_config(model.config) if model.config else None
            ),
            optimizer_status=cls.convert_optimizer_status(model.optimizer_status),
            payload_schema=(
            ...
            status=cls.convert_collection_status(model.status),
            vectors_count=(
                model.vectors_count if model.vectors_count is not None else None
            ),
            points_count=model.points_count,
        )
    ...
    @classmethod
    def convert_collection_status(
        cls, model: rest.CollectionStatus
    ) -> grpc.CollectionStatus:
    ...
    @classmethod
    def convert_optimizer_status(
        cls, model: rest.OptimizersStatus
    ) -> grpc.OptimizerStatus:
    ...
    def convert_payload_schema(
        ...
        return dict(
            (key, cls.convert_payload_index_info(val)) for key, val in model.items()
        )

    @classmethod
    def convert_payload_index_info(
        cls, model: rest.PayloadIndexInfo
    ) -> grpc.PayloadSchemaInfo:
        params = model.params
        return grpc.PayloadSchemaInfo(
            data_type=cls.convert_payload_schema_type(model.data_type),
            params=(
                cls.convert_payload_schema_params(params)
    ...
    def convert_payload_schema_params(
        ...
        if isinstance(model, rest.TextIndexParams):
            return grpc.PayloadIndexParams(
                text_index_params=cls.convert_text_index_params(model)
            )
        ...
        if isinstance(model, rest.GeoIndexParams):
            return grpc.PayloadIndexParams(
                geo_index_params=cls.convert_geo_index_params(model)
            )

        if isinstance(model, rest.BoolIndexParams):
            return grpc.PayloadIndexParams(
                bool_index_params=cls.convert_bool_index_params(model)
            )
        ...
        if isinstance(model, rest.UuidIndexParams):
            return grpc.PayloadIndexParams(
                uuid_index_params=cls.convert_uuid_index_params(model)
            )

        raise ValueError(
            f"invalid PayloadSchemaParams model: {model}"
        )  # pragma: no cover

    @classmethod
    def convert_payload_schema_type(
        cls, model: rest.PayloadSchemaType
    ) -> grpc.PayloadSchemaType:
    ...
    def convert_scored_point(cls, model: rest.ScoredPoint) -> grpc.ScoredPoint:
        return grpc.ScoredPoint(
            ...
            version=model.version,
            shard_key=(
                cls.convert_shard_key(model.shard_key) if model.shard_key else None
            ),
            order_value=(
                cls.convert_order_value(model.order_value)
                if model.order_value
                else None
            ),
        )
    ...
    def convert_field_condition(cls, model: rest.FieldCondition) -> grpc.FieldCondition:
        if model.match is not None:
            return grpc.FieldCondition(
                key=model.key, match=cls.convert_match(model.match)
            )
        if model.range is not None:
            if isinstance(model.range, rest.Range):
                return grpc.FieldCondition(
                    key=model.key, range=cls.convert_range(model.range)
                )
    ...
    @classmethod
    def convert_collection_config(
        cls, model: rest.CollectionConfig
    ...
    @classmethod
    def convert_collection_params(
        cls, model: rest.CollectionParams
    ) -> grpc.CollectionParams:
        return grpc.CollectionParams(
            vectors_config=(
                cls.convert_vectors_config(model.vectors)
    ...
    def convert_max_optimization_threads(
        ...
        if model == rest.MaxOptimizationThreadsSetting.AUTO:
            return grpc.MaxOptimizationThreads(
    ...
    @classmethod
    def convert_optimizers_config(
        cls, model: rest.OptimizersConfig
    ...
    def convert_match(cls, model: rest.Match) -> grpc.Match:
        ...
        if isinstance(model, rest.MatchExcept):
            ...
            if isinstance(model.except_[0], str):
                return grpc.Match(
                    except_keywords=grpc.RepeatedStrings(strings=model.except_)
                )
            if isinstance(model.except_[0], int):
                return grpc.Match(
                    except_integers=grpc.RepeatedIntegers(integers=model.except_)
                )
    ...
    @classmethod
    def convert_alias_operations(
        cls, model: rest.AliasOperations
    ) -> grpc.AliasOperations:
        if isinstance(model, rest.CreateAliasOperation):
            return grpc.AliasOperations(
                create_alias=cls.convert_create_alias(model.create_alias)
            )
        if isinstance(model, rest.DeleteAliasOperation):
            return grpc.AliasOperations(
                delete_alias=cls.convert_delete_alias(model.delete_alias)
            )
        if isinstance(model, rest.RenameAliasOperation):
            return grpc.AliasOperations(
                rename_alias=cls.convert_rename_alias(model.rename_alias)
            )
    ...
    @classmethod
    def convert_alias_description(
        cls, model: rest.AliasDescription
    ) -> grpc.AliasDescription:
    ...
    @classmethod
    def convert_recommend_example(
        cls, model: rest.RecommendExample
    ) -> grpc.VectorExample:
    ...
    @classmethod
    def convert_sparse_vector_to_vector_output(
        cls, model: rest.SparseVector
    ) -> grpc.VectorOutput:
    ...
    @classmethod
    def convert_multivector_config(
        cls, model: rest.MultiVectorConfig
    ) -> grpc.MultiVectorConfig:
        ...
        if isinstance(model, list):
            ...
            for key, val in model.items():
                if isinstance(val, list):
                    ...
                    vectors.update(
                        {key: grpc.Vector(document=cls.convert_document(val))}
                    )
                    ...
                    vectors.update(
                        {key: grpc.Vector(object=cls.convert_inference_object(val))}
                    )
            return grpc.Vectors(vectors=grpc.NamedVectors(vectors=vectors))
        elif isinstance(model, rest.Document):
            return grpc.Vectors(
                vector=grpc.Vector(document=cls.convert_document(model))
            )
            ...
            return grpc.Vectors(
                vector=grpc.Vector(object=cls.convert_inference_object(model))
            )
        else:
            raise ValueError(f"invalid VectorStruct model: {model}")  # pragma: no cover

    @classmethod
    def convert_vector_struct_output(
        cls, model: rest.VectorStructOutput
    ) -> grpc.VectorsOutput:
        def convert_vector(
            vector: Union[list[float], list[list[float]]],
        ) -> grpc.VectorOutput:
        ...
        if isinstance(model, list):
            ...
            for key, val in model.items():
                if isinstance(val, list):
                    ...
                    vectors.update(
                        {key: cls.convert_sparse_vector_to_vector_output(val)}
                    )
            return grpc.VectorsOutput(vectors=grpc.NamedVectorsOutput(vectors=vectors))
        else:
            raise ValueError(
                f"invalid VectorStructOutput model: {model}"
            )  # pragma: no cover
    ...
    def convert_batch_vector_struct(
        ...
        if isinstance(model, list):
            ...
            raise ValueError(
                f"invalid BatchVectorStruct model: {model}"
            )  # pragma: no cover
    ...
    def convert_named_vector_struct(
        ...
        if isinstance(model, list):
            ...
            raise ValueError(
                f"invalid NamedVectorStruct model: {model}"
            )  # pragma: no cover
    ...
    @classmethod
    def convert_multi_dense_vector(
        cls, model: list[list[float]]
    ) -> grpc.MultiDenseVector:
    ...
    def convert_document(cls, model: rest.Document) -> grpc.Document:
        return grpc.Document(
            ...
            model=model.model,
            options=(
                payload_to_grpc(model.options) if model.options is not None else None
            ),
        )
    ...
    @classmethod
    def convert_inference_object(
        cls, model: rest.InferenceObject
    ) -> grpc.InferenceObject:
        ...
        if isinstance(model, list):
            if len(model) != 0 and isinstance(
                ...
                return grpc.VectorInput(
                    multi_dense=cls.convert_multi_dense_vector(model)
                )
    ...
    @classmethod
    def convert_context_input_pair(
        cls, model: rest.ContextPair
    ) -> grpc.ContextInputPair:
    ...
    def convert_mmr(cls, model: rest.Mmr) -> grpc.Mmr:
        return grpc.Mmr(
            diversity=model.diversity, candidates_limit=model.candidates_limit
        )
        ...
        if isinstance(model, rest.NearestQuery):
            if model.mmr is not None:
                nearest_with_mmr = grpc.NearestInputWithMmr(
                    nearest=cls.convert_vector_input(model.nearest),
                    mmr=cls.convert_mmr(model.mmr),
                )
    ...
    def convert_formula_query(cls, model: rest.FormulaQuery) -> grpc.Formula:
        defaults = (
            payload_to_grpc(model.defaults) if model.defaults is not None else None
        )
    ...
    def convert_prefetch_query(cls, model: rest.Prefetch) -> grpc.PrefetchQuery:
        ...
        return grpc.PrefetchQuery(
            prefetch=prefetch,
            query=(
                cls.convert_query_interface(model.query)
                if model.query is not None
                else None
            ),
            using=model.using if model.using is not None else None,
            filter=(
                cls.convert_filter(model.filter) if model.filter is not None else None
            ),
            params=(
                cls.convert_search_params(model.params)
                if model.params is not None
    ...
    def convert_discover_request(
        ...
        query_filter = (
            None if model.filter is None else cls.convert_filter(model=model.filter)
        )

        search_params = (
            None if model.params is None else cls.convert_search_params(model.params)
        )
        ...
        with_vectors = (
            None
            if model.with_vector is None
            else cls.convert_with_vectors(model.with_vector)
        )

        lookup_from = (
            None
            if model.lookup_from is None
            else cls.convert_lookup_location(model.lookup_from)
        )

        shard_key_selector = (
            None
            if model.shard_key is None
            else cls.convert_shard_key_selector(model.shard_key)
        )
    ...
    def convert_tokenizer_type(cls, model: rest.TokenizerType) -> grpc.TokenizerType:
        if model == rest.TokenizerType.WORD:
            ...
            raise ValueError(
                f"invalid TokenizerType model: {model}"
            )  # pragma: no cover

    @classmethod
    def convert_text_index_params(
        cls, model: rest.TextIndexParams
    ) -> grpc.TextIndexParams:
    ...
    def convert_stopwords(cls, model: rest.StopwordsInterface) -> grpc.StopwordsSet:
        ...
        if isinstance(model, rest.StopwordsSet):
            return grpc.StopwordsSet(
                languages=(
                    [lang for lang in model.languages] if model.languages else None
                ),
                custom=model.custom,
            )

        raise ValueError(
            f"invalid StopwordsInterface model: {model}"
        )  # pragma: no cover
    ...
    def convert_stemmer(cls, model: rest.StemmingAlgorithm) -> grpc.StemmingAlgorithm:
        if isinstance(model, rest.SnowballParams):
            return grpc.StemmingAlgorithm(
                snowball=grpc.SnowballParams(language=model.language)
            )
    ...
    @classmethod
    def convert_float_index_params(
        cls, model: rest.FloatIndexParams
    ) -> grpc.FloatIndexParams:
        return grpc.FloatIndexParams(
            is_principal=model.is_principal, on_disk=model.on_disk
        )

    @classmethod
    def convert_geo_index_params(
        cls, model: rest.GeoIndexParams
    ) -> grpc.GeoIndexParams:
        return grpc.GeoIndexParams(on_disk=model.on_disk)

    @classmethod
    def convert_bool_index_params(
        cls, model: rest.BoolIndexParams
    ) -> grpc.BoolIndexParams:
    ...
    @classmethod
    def convert_uuid_index_params(
        cls, model: rest.UuidIndexParams
    ) -> grpc.UuidIndexParams:
    ...
    @classmethod
    def convert_read_consistency(
        cls, model: rest.ReadConsistency
    ) -> grpc.ReadConsistency:
        if isinstance(model, int):
    ...
    def convert_write_ordering(cls, model: rest.WriteOrdering) -> grpc.WriteOrdering:
        if model == rest.WriteOrdering.WEAK:
            ...
            raise ValueError(
                f"invalid WriteOrdering model: {model}"
            )  # pragma: no cover
    ...
    @classmethod
    def convert_compression_ratio(
        cls, model: rest.CompressionRatio
    ) -> grpc.CompressionRatio:
        if model == rest.CompressionRatio.X4:
            ...
            raise ValueError(
                f"invalid CompressionRatio model: {model}"
            )  # pragma: no cover
    ...
    def convert_point_group(cls, model: rest.PointGroup) -> grpc.PointGroup:
        return grpc.PointGroup(
            ...
            lookup=(
                cls.convert_record(model.lookup) if model.lookup is not None else None
            ),
        )
    ...
    @classmethod
    def convert_vector_params_diff(
        cls, model: rest.VectorParamsDiff
    ...
    @classmethod
    def convert_vectors_config_diff(
        cls, model: rest.VectorsConfigDiff
    ) -> grpc.VectorsConfigDiff:
        if isinstance(model, dict) and len(model) == 1 and "" in model:
            return grpc.VectorsConfigDiff(
                params=cls.convert_vector_params_diff(model[""])
            )
        elif isinstance(model, dict):
            return grpc.VectorsConfigDiff(
                params_map=grpc.VectorParamsDiffMap(
                    map=dict(
                        (key, cls.convert_vector_params_diff(val))
                        for key, val in model.items()
                    )
                )
            )
    ...
    def convert_point_insert_operation(
        ...
        if isinstance(model, rest.PointsBatch):
            ...
            raise ValueError(
                f"invalid PointInsertOperations model: {model}"
            )  # pragma: no cover

    @classmethod
    def convert_update_operation(
    ...
    def convert_points_update_operation(
        ...
        if isinstance(model, rest.UpsertOperation):
            ...
            if model.set_payload.points:
                ...
                raise ValueError(
                    f"invalid SetPayloadOperation model: {model}"
                )  # pragma: no cover
            ...
            if model.overwrite_payload.points:
                points_selector = rest.PointIdsList(
                    points=model.overwrite_payload.points
                )
            elif model.overwrite_payload.filter:
                points_selector = rest.FilterSelector(
                    filter=model.overwrite_payload.filter
                )
            else:
                raise ValueError(
            ...
            if model.delete_payload.points:
                ...
                points_selector = rest.FilterSelector(
                    filter=model.delete_payload.filter
                )
            else:
                raise ValueError(
            ...
            return grpc.PointsUpdateOperation(
                update_vectors=grpc.PointsUpdateOperation.UpdateVectors(
                    points=[
                        cls.convert_point_vectors(point)
                        for point in model.update_vectors.points
                    ],
                    shard_key_selector=shard_key_selector,
                )
            )
            ...
            if model.delete_vectors.points:
                ...
                points_selector = rest.FilterSelector(
                    filter=model.delete_vectors.filter
                )
            else:
                raise ValueError(
                    f"invalid DeletePayloadOperation model: {model}"
    ...
    @classmethod
    def convert_recommend_strategy(
        cls, model: rest.RecommendStrategy
    ) -> grpc.RecommendStrategy:
    ...
    @classmethod
    def convert_sparse_index_params(
        cls, model: rest.SparseIndexParams
    ) -> grpc.SparseIndexConfig:
    ...
    def convert_sparse_vector_params(
        ...
        return grpc.SparseVectorParams(
            index=(
                cls.convert_sparse_index_params(model.index)
    ...
    @classmethod
    def convert_shard_key_selector(
        cls, model: rest.ShardKeySelector
        ...
        if isinstance(model, list):
            return grpc.ShardKeySelector(
                shard_keys=[cls.convert_shard_key(key) for key in model]
            )
    ...
    def convert_sharding_method(cls, model: rest.ShardingMethod) -> grpc.ShardingMethod:
        if model == rest.ShardingMethod.AUTO:
            ...
            raise ValueError(
                f"invalid ShardingMethod model: {model}"
            )  # pragma: no cover

    @classmethod
    def convert_health_check_reply(
        cls, model: rest.VersionInfo
    ) -> grpc.HealthCheckReply:
    ...
    @classmethod
    def convert_search_matrix_pair(
        cls, model: rest.SearchMatrixPair
    ...
    @classmethod
    def convert_strict_mode_config(
        cls, model: rest.StrictModeConfig
