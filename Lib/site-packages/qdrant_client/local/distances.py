...
class RecoQuery:
    def __init__(
        ...
        self.positive: list[types.NumpyArray] = [
            np.array(vector) for vector in positive
        ]
        self.negative: list[types.NumpyArray] = [
            np.array(vector) for vector in negative
        ]

        assert not np.isnan(
            self.positive
        ).any(), "Positive vectors must not contain NaN"
        assert not np.isnan(
            self.negative
        ).any(), "Negative vectors must not contain NaN"
...
def cosine_similarity(
    query: types.NumpyArray, vectors: types.NumpyArray
) -> types.NumpyArray:
    """
...
def calculate_distance_core(
    ...
    if distance_type == models.Distance.EUCLID:
        return -np.square(vectors - query, dtype=np.float32).sum(
