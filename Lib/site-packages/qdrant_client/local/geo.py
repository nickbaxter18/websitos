...
def test_geo_distance() -> None:
    ...
    assert (
        geo_distance(moscow["lon"], moscow["lat"], moscow["lon"], moscow["lat"]) < 1.0
    )

    assert (
        geo_distance(moscow["lon"], moscow["lat"], london["lon"], london["lat"])
        > 2400 * 1000
    )
    assert (
        geo_distance(moscow["lon"], moscow["lat"], london["lon"], london["lat"])
    ...
    assert (
        geo_distance(moscow["lon"], moscow["lat"], berlin["lon"], berlin["lat"])
        > 1600 * 1000
    )
    assert (
        geo_distance(moscow["lon"], moscow["lat"], berlin["lon"], berlin["lat"])
