...
def check_values_count(
    condition: models.ValuesCount, values: Optional[list[Any]]
) -> bool:
    if values is None:
        return False
...
def check_geo_bounding_box(condition: models.GeoBoundingBox, values: Any) -> bool:
    if isinstance(values, dict) and "lat" in values and "lon" in values:
        ...
        if condition.top_left.lon > condition.bottom_right.lon:
            longitude_condition = (
                condition.top_left.lon <= lon <= 180
                or -180 <= lon <= condition.bottom_right.lon
            )
        else:
            longitude_condition = (
                condition.top_left.lon <= lon <= condition.bottom_right.lon
            )
...
def check_geo_polygon(condition: models.GeoPolygon, values: Any) -> bool:
    if isinstance(values, dict) and "lat" in values and "lon" in values:
        ...
        return boolean_point_in_polygon(
            point=(lat, lon), exterior=exterior, interiors=interiors
        )
...
def check_nested_filter(nested_filter: models.Filter, values: list[Any]) -> bool:
    return any(
        check_filter(nested_filter, v, point_id=-1, has_vector={}) for v in values
    )
    ...
    if isinstance(condition, models.IsNullCondition):
        ...
        if condition.geo_bounding_box is not None:
            if values is None:
                return False
            return any(
                check_geo_bounding_box(condition.geo_bounding_box, v) for v in values
            )
...
def check_must(
    ...
    return all(
        check_condition(condition, payload, point_id, has_vector)
...
def check_must_not(
    ...
    return all(
        not check_condition(condition, payload, point_id, has_vector)
        for condition in conditions
    )
...
def check_min_should(
    ...
    return (
        sum(
            check_condition(condition, payload, point_id, vectors)
            for condition in conditions
        )
        >= min_count
    )
...
def check_filter(
    ...
    if payload_filter.must_not is not None:
        if not check_must_not(
            ensure_condition_list(payload_filter.must_not),
            payload,
            point_id,
            has_vector,
        ):
