...
class LocalCollection:
    ...
    def __init__(
        ...
        self.ids: dict[models.ExtendedPointId, int] = (
            {}
        )  # Mapping from external id to internal id
        self.ids_inv: list[models.ExtendedPointId] = (
            []
        ...
        if location is not None:
            self.storage = CollectionPersistence(
                location, force_disable_check_same_thread
            )
        self.load_vectors()
    ...
    def _resolve_query_vector_name(
        ...
        str,
        Union[DenseQueryVector, SparseQueryVector, MultiQueryVector, types.NumpyArray],
    ]:
        ...
        vector: Union[
            DenseQueryVector, SparseQueryVector, MultiQueryVector, types.NumpyArray
        ]
    ...
    def _get_vectors(
        ...
        if isinstance(with_vectors, list):
            all_vectors = {
                name: all_vectors[name] for name in with_vectors if name in all_vectors
            }
        ...
        if isinstance(query_vector, np.ndarray):
            ...
            if rescore_idf:
                query_vector = query_vector.transform_sparse(
                    lambda x: self._rescore_idf(x, name)
                )
            if query_vector.strategy == models.RecommendStrategy.BEST_SCORE:
            ...
            if query_vector.strategy == models.RecommendStrategy.BEST_SCORE:
                scores = calculate_multi_recommend_best_scores(
                    query_vector, vectors, distance
                )
            elif query_vector.strategy == models.RecommendStrategy.SUM_SCORES:
                scores = calculate_multi_recommend_sum_scores(
                    query_vector, vectors, distance
                )
            else:
                raise TypeError(
    ...
    def _prefetch(self, prefetch: types.Prefetch) -> list[types.ScoredPoint]:
        ...
        if len(inner_prefetches) > 0:
            sources = [
                self._prefetch(inner_prefetch) for inner_prefetch in inner_prefetches
            ]
    ...
    def _merge_sources(
        ...
        if isinstance(query, models.FusionQuery):
            ...
            if len(sources_ids) == 0:
                ...
                filter_with_sources = _include_ids_in_filter(
                    query_filter, list(sources_ids)
                )
                return self._query_collection(
                    query=query,
                    using=using,
    ...
    def query_groups(
        ...
        for point in points.points:
            ...
            group_values = list(
                set(v for v in group_values if isinstance(v, (str, int)))
            )
    ...
    def _preprocess_recommend_input(
            ...
            for example in examples:
                if isinstance(example, get_args(types.PointId)):
                    if example not in collection.ids:
                        raise ValueError(
                            f"Point {example} is not found in the collection"
                        )
        ...
        collection = (
            lookup_from_collection if lookup_from_collection is not None else self
        )
    ...
    def _recommend_average_dense(
        ...
        negative_vectors_np = (
            np.stack(negative_vectors) if len(negative_vectors) > 0 else None
        )
        ...
        if negative_vectors_np is not None:
            vector = (
                ...
                + mean_positive_vector
                - np.mean(negative_vectors_np, axis=0)
            )
        else:
    ...
    def _recommend_average_sparse(
        ...
        if negative_vectors:
            ...
            vector = merge_positive_and_negative_avg(
                mean_positive_vector, mean_negative_vector
            )
        else:
            vector = mean_positive_vector
        return vector
    ...
    def _recommend_average_multi(
        positive_vectors: list[list[list[float]]],
        negative_vectors: list[list[list[float]]],
    ) -> list[list[float]]:
        recommend_vector = [
            vector for multi_vector in positive_vectors for vector in multi_vector
        ]
    ...
    def _construct_recommend_query(
        ...
        if strategy == types.RecommendStrategy.AVERAGE_VECTOR:
            ...
            if positive_vectors:
                ...
                raise ValueError(
                    "No positive examples given with 'average_vector' strategy"
                )
    ...
    def _preprocess_target(
        target: Optional[models.VectorInput],
    ...
    def _preprocess_context(
        self,
        context: list[models.ContextPair],
        collection: "LocalCollection",
        vector_name: str,
    ) -> tuple[
        list[ContextPair],
        ...
        for pair in context:
            ...
            if isinstance(pair_vectors[0], SparseVector) and isinstance(
                ...
                sparse_context_vectors.append(
                    SparseContextPair(
                        positive=pair_vectors[0], negative=pair_vectors[1]
                    )
                )
            elif isinstance(pair_vectors[0], list) and isinstance(
                pair_vectors[1], list
            ):
                if isinstance(pair_vectors[0][0], float) and isinstance(
                    pair_vectors[1][0], float
                ):
                    dense_context_vectors.append(
                        ContextPair(positive=pair_vectors[0], negative=pair_vectors[1])
                    )
                elif isinstance(pair_vectors[0][0], list) and isinstance(
                    pair_vectors[1][0], list
                ):
    ...
    def _preprocess_discover(
        ...
        target_vector, target_id = self._preprocess_target(
            target, collection, vector_name
        )
            ...
            if isinstance(target_vector, list):
                if isinstance(target_vector[0], float):
                    ...
                    query_vector = MultiDiscoveryQuery(
                        target_vector, multi_context_vectors
                    )
            elif isinstance(target_vector, SparseVector):
                query_vector = SparseDiscoveryQuery(
                    target_vector, sparse_context_vectors
                )
            else:
    ...
    def _scroll_by_id(
        ...
        for point_id, idx in sorted_ids:
            if offset is not None and self._universal_id(point_id) < self._universal_id(
                offset
            ):
                continue
    ...
    def _scroll_by_value(
        ...
        direction = (
            order_by.direction
            if order_by.direction is not None
    ...
    def _search_with_mmr(
        ...
        search_limit = (
            mmr.candidates_limit if mmr.candidates_limit is not None else limit
        )
        using = using or DEFAULT_VECTOR_NAME
    ...
    def _mmr(
        ...
        distance = (
            self.get_vector_params(using).distance
            if using not in self.sparse_vectors
            else None
        )
        query_vector = (
        ...
        for candidate_id, candidate_vector in zip(candidate_ids, candidate_vectors):
            if isinstance(candidate_vector, list) and isinstance(
                candidate_vector[0], float
            ):
                ...
                if not isinstance(candidate_vectors[0], np.ndarray):
                    candidate_vectors = [
                        np.array(multivec) for multivec in candidate_vectors
                    ]
        ...
        while len(selected) < limit and len(pending) > 0:
            ...
            for pending_id in pending:
                ...
                mmr_score = (
                    lambda_ * relevance_score
                    - (1.0 - lambda_) * max_similarity_to_selected
                )
    ...
    def _rescore_with_formula(
        ...
        prefetches_scores = [
            dict((point.id, point.score) for point in prefetch)
    ...
    def _add_point(self, point: models.PointStruct) -> None:
        ...
        assert len(self.payload) == len(
            self.ids_inv
        ), "Payload and ids_inv must be the same size"
        ...
        for vector_name, named_vectors in self.vectors.items():
            ...
            if named_vectors.shape[0] <= idx:
                named_vectors = np.resize(
                    named_vectors, (idx * 2 + 1, named_vectors.shape[1])
                )
        ...
        if isinstance(point.vector, dict):
            ...
            for vector_name, vector in point.vector.items():
                if vector_name not in self._all_vectors_keys:
                    raise ValueError(
                        f"Wrong input: Not existing vector name error: {vector_name}"
                    )
    ...
    def _update_named_vectors(
        self,
        idx: int,
        vectors: dict[str, Union[list[float], SparseVector, list[list[float]]]],
    ) -> None:
    ...
    def _filter_to_ids(
        self, delete_filter: types.Filter
    ) -> list[models.ExtendedPointId]:
    ...
    def set_payload(
        ...
        keys: Optional[list[JsonPathItem]] = (
        ...
        for update_op in update_operations:
            if isinstance(update_op, models.UpsertOperation):
                if isinstance(update_op.upsert, models.PointsBatch):
                    ...
                    raise ValueError(
                        f"Unsupported upsert type: {type(update_op.upsert)}"
                    )
                ...
                points_selector = (
                    update_op.set_payload.points or update_op.set_payload.filter
                )
                self.set_payload(
                    update_op.set_payload.payload,
                    points_selector,
                    update_op.set_payload.key,
                )
            elif isinstance(update_op, models.OverwritePayloadOperation):
                points_selector = (
                    update_op.overwrite_payload.points
                    or update_op.overwrite_payload.filter
                )
                self.overwrite_payload(
                    update_op.overwrite_payload.payload, points_selector
                )
...
def set_prefetch_limit_recursively(
    prefetch: types.Prefetch, limit: int
) -> types.Prefetch:
    if prefetch is not None:
        if isinstance(prefetch.prefetch, list):
            return types.Prefetch(
                limit=limit,
                prefetch=[
                    set_prefetch_limit_recursively(p, limit) for p in prefetch.prefetch
                ],
            )
        else:
