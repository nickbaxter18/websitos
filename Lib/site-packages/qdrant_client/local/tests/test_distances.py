...
def test_distances() -> None:
    ...
    assert np.allclose(
        calculate_distance(query, vectors, models.Distance.DOT), [14.0, 14.0]
    )
    assert np.allclose(
        calculate_distance(query, vectors, models.Distance.EUCLID), [0.0, 0.0]
    )
    assert np.allclose(
        calculate_distance(query, vectors, models.Distance.MANHATTAN), [0.0, 0.0]
    )
    # cosine modifies vectors inplace
    assert np.allclose(
        calculate_distance(query, vectors, models.Distance.COSINE), [1.0, 1.0]
    )
    ...
    assert calculate_distance_sparse(sparse_query, sparse_vectors) == [
        np.float32("-inf")
    ]
    ...
    assert (
        calculate_multi_distance(multivector_query, docs, models.Distance.DOT)[0]
        == 40.0
    )
