    ...
    if isinstance(expression, (float, int)):  # Constant
        ...
        for expr in expression.mult:
            factor = evaluate_expression(
                expr, point_id, scores, payload, has_vector, defaults
            )
        ...
        return abs(
            evaluate_expression(
                expression.abs, point_id, scores, payload, has_vector, defaults
            )
        )
        ...
        value = evaluate_expression(
            expression.ln, point_id, scores, payload, has_vector, defaults
        )
        ...
        if dt is None:
            raise ValueError(
                f"Expected datetime in supported format for {expression.datetime}"
            )
...
def evaluate_decay_params(
    ...
    midpoint = (
        params.midpoint if params.midpoint is not None else DEFAULT_DECAY_MIDPOINT
    )
...
def try_extract_payload_value(
    key: str, payload: models.Payload, defaults: dict[str, Any]
) -> Any:
    # Get value from payload
    ...
    if value is None:
        raise ValueError(
            f"No value found for {key} in the payload nor the formula defaults"
        )
