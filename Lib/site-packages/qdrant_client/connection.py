    ...
    def intercept_call(
        ...
        if auth_token_provider:
            if not asyncio.iscoroutinefunction(auth_token_provider):
                ...
                raise ValueError(
                    "Synchronous channel requires synchronous auth token provider."
                )
...
def parse_channel_options(
    ...
    _options = [
        (option_name, option_value) for option_name, option_value in options.items()
    ]
...
def get_channel(
    ...
    if ssl:
        ...
        channel = grpc.secure_channel(
            f"{host}:{port}", ssl_creds, _options, compression
        )
