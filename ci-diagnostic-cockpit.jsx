...
import { Loader2, CheckCircle, XCircle, Activity, Download, AlertTriangle, RefreshCw, Clock, Info, ChevronDown, ChevronUp } from "lucide-react";
import {
  LineChart,
  Line,
  XAxis,
...
export default function DiagnosticCockpit() {
    ...
    async function fetchWorkflows() {
      try {
        const res = await fetch(
          "https://api.github.com/repos/nickbaxter18/websitos/actions/runs",
          {
            headers: {
  ...
  function getPriorityLabel(labels) {
    ...
    if (labels.find((l) => l.name === "P1")) return { text: "P1 - Critical", color: "text-red-600" };
    if (labels.find((l) => l.name === "P2")) return { text: "P2 - High", color: "text-yellow-600" };
    if (labels.find((l) => l.name === "P3")) return { text: "P3 - Medium", color: "text-green-600" };
    return null;
  }
  ...
  function getGlobalStatus() {
    ...
    if (failingWorkflows.length > 0) return { text: "Critical workflow failures detected", color: "bg-red-700", target: "#workflows", count: failingWorkflows.length };
    if (criticalIssues.length > 0) return { text: "Critical issues open", color: "bg-red-600", target: "#issues", count: criticalIssues.length };
    if (highIssues.length > 0) return { text: "High priority issues open", color: "bg-yellow-500", target: "#issues", count: highIssues.length };
    if (mediumIssues.length > 0) return { text: "Medium issues open", color: "bg-green-500", target: "#issues", count: mediumIssues.length };
  ...
  return (
    <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      <div className={`col-span-full text-white px-4 py-2 rounded mb-4 ${globalStatus.color}`}>
        <div className="flex justify-between items-center">
          ...
          <div className="flex space-x-2">
            {globalStatus.target && (
              <Button variant="secondary" size="sm" onClick={() => scrollToTarget(globalStatus.target)}>
        ...
        {showDetails && (
          <div className="mt-2 bg-white text-black rounded p-2 text-sm max-h-40 overflow-y-auto">
