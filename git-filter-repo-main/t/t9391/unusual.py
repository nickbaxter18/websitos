...
def track_everything(obj, *_ignored):
    ...
    if type(obj) == fr.Reset:
        ...
        def assert_not_reached(x):
            raise SystemExit("should have been skipped!")
...
parser.run(input=sys.stdin.detach(), output=open(os.devnull, "bw"))
...
print(
    "Found {} blobs/commits and {} other objects".format(
        total_objects["common"], total_objects["uncommon"]
    )
)
...
args = fr.FilteringOptions.parse_args(["--stdin", "--dry-run", "--path", "salutation"])
